...
def transform_input(responses):
    ...
    img3d = np.reshape(img1d, (responses[0].height, responses[0].width, 3))

    from PIL import Image
    image = Image.fromarray(img3d)
...
def dist_from(point,magnitude=True):
    val = (((((point[0] - origin[0])** 2) + ((point[1] - origin[1])** 2))**(1/2)) - radius)
    if magnitude:
        return max(val, -val)
    return val
...
def compute_reward(car_state,distance, angle, record=False):
    ...
    dist = dist_from(car_pt,magnitude=True)
    ...
    if dist > thresh_dist:
        ...
        #+ distance*5
        if record:
            line = '%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n' % (car_pt[0]-origin[0],car_pt[1]-origin[1],car_state.speed,reward_speed,reward_dist,reward_angle,reward)
...
def compute_angle(car_state, last_pos):
    cs = car_state.kinematics_estimated.position
    pos = [cs.x_val, cs.y_val]
    try:
        tangent_angle = math.atan(-(pos[0]-origin[0])/(pos[1]-origin[1]))*180/math.pi
        current_angle = math.atan((pos[1]-last_pos[1])/(pos[0]-last_pos[0]))* 180/math.pi
    except:
        tangent_angle = 0
        current_angle = 0
    print('tangent_angle: %.1f current_angle: %.1f' %(tangent_angle, current_angle))
    if max(tangent_angle-current_angle,current_angle-tangent_angle) < 90:
        return tangent_angle - current_angle
    else:
        if -90 < 180 - (tangent_angle-current_angle) < 90:
            return -(180 - (tangent_angle-current_angle))
        return -(180 + tangent_angle-current_angle)
...
try:
    ...
    if record:
        dirname = time.strftime("%Y_%m_%d_%H_%M") + '_dqn' 
        os.mkdir(dirname)
    ...
    while True:
        ...
        reward = compute_reward(car_state,distance, angle,record)
        ...
        #responses = client.simGetImages([airsim.ImageRequest("0", airsim.ImageType.DepthPerspective, True, False)])
        cs = client.getCarState()
        angle = compute_angle(cs, last_pos)
        #current_state = transform_input(responses)
