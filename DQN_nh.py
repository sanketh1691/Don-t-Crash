...
def line_equation(xs, ys):
    ...
    return m+0.00000001,c+0.00000001
...
def compute_angle(car_state, last_pos, line):
    ...
    try:
        line_angle = math.atan(-(pts[line][1][1]-pts[line][0][1])/(pts[line][1][0]-pts[line][0][0]))*180/math.pi
...
'''
z=0
nh_pts = [np.array([0, -1, z]), np.array([130, -1, z]), np.array([130, 125, z]), np.array([0, 125, z]), np.array([0, -1, z]), np.array([130, -1, z]), np.array([130, -128, z]), np.array([0, -128, z]), np.array([0, -1, z])]
pts = []

for i in range(len(nh_pts)-1):
    pts.append([[nh_pts[i][0],nh_pts[i+1][0]],[nh_pts[i][1],nh_pts[i+1][1]]])
...
try:
    ...
    if record:
        dirname = time.strftime("%Y_%m_%d_%H_%M") + '_dqn_nh' 
        os.mkdir(dirname)
